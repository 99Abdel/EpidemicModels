...
y0 = [y(FIRST,1),0,y(FIRST,2),y(FIRST,3)];
...
hold on
plot(F(:,1), 'm-');
legend({'Data points S', 'Fitted Curve S'})

nexttile
hold on
plot(F(:,2), 'g-');
legend({'Fitted Curve E'})
...
hold on
plot(F(:,3), 'r-');
legend({'Data points I', 'Fitted Curve I'})

nexttile
plot(tg,D(:,3), 'b+');
hold on
plot(F(:,4), 'y-');
legend({'Data points R', 'Fitted Curve R'})
